...
const HorizontalCards = ({ data }) => {
  ...
  return (
    <div className="w-full h-auto sm:h-[45vh] p-5 mb-5 flex flex-col">
      <div className="w-full flex overflow-x-auto mb-5 p-5 space-x-5 scrollbar-hide">
        {data.length > 0 ? (
          data.map((d, i) => (
            <div
              key={i}
              className="h-auto sm:h-[40vh] min-w-[180px] sm:min-w-[240px] max-w-[45%] sm:max-w-[20%] bg-gray-100 rounded-xl shadow-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl"
            >
              <Link to={`/details/${d.media_type}/${d.id}`}>
                <img
                  className="w-full h-[140px] sm:h-[180px] object-cover rounded-t-xl"
                  src={`https://image.tmdb.org/t/p/original/${d.backdrop_path || d.poster_path}`}
                  alt="Image Not Available"
                />
              </Link>
              <div className="p-4 text-black flex flex-col justify-between">
                ...
                <div className="text-xs sm:text-sm text-gray-700 flex-grow overflow-hidden">
                  <p className="line-clamp-3">
                    {d.overview.length > 120 ? `${d.overview.slice(0, 120)}...` : d.overview}
                  </p>
                  {d.overview.length > 120 && (
          ...
          <h1 className="text-white text-2xl sm:text-3xl font-extrabold mx-auto">Nothing to show</h1>
        )}
      </div>
      <style>
        {`
