...
const Header = ({ data }) => {
  return (
    <div
      ...
      className="h-[70vh] flex flex-col justify-end items-start p-6 sm:p-10 md:p-16 border-2 border-gray-700 rounded-xl"
    >
      <h1 className="w-full sm:w-[80%] md:w-[70%] text-3xl sm:text-4xl md:text-5xl font-extrabold text-white drop-shadow-lg">
        {data.name || data.title || data.original_name || data.original_title}
      </h1>

      <p className="mt-3 text-sm sm:text-lg md:text-xl w-full sm:w-[80%] md:w-[70%] text-gray-300 leading-6 sm:leading-7 drop-shadow-md">
        {data.overview.slice(0, 180)}...
        <Link to={`/${data.media_type}/details/${data.id}`} className="text-blue-400 text-base sm:text-lg font-semibold">
          more
        </Link>
      </p>
      ...
      <Link className="mt-5 sm:mt-6 px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-[#6f689f] to-[#4b4788] text-white text-sm sm:text-lg font-semibold rounded-lg shadow-xl tracking-wider border-2 border-gray-500 transition-all duration-300 hover:shadow-2xl hover:border-white hover:from-[#867cc1] hover:to-[#6258b0]">
        Watch Trailer
      </Link>
    </div>
  );
